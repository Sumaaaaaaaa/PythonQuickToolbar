...
| `returnType` | type description❓ | N | `None` | Indicate to quickToolbar what the data type returned by the method is, and quickToolbar will create a window for displaying the data based on that data type, or not if it's `None`, see [here](#executionmode--returntype) for details. | ``returnType=str`` |
